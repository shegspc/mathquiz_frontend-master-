    ...
    const updateResponse = (event) => {
      ...
      const inputKeyPress = (event) => {
        if (event.key === "Enter") {
          ...
          if (answer === inputOne + inputTwo * inputThree) {
            //Answer is correct
            setScore(score + 1);
            setInputOne(Math.ceil(Math.random() * 12));
            setInputTwo(Math.ceil(Math.random() * 12));
            setInputThree(Math.ceil(Math.random() * 12));
    ...
    return <div className="Quiz fadeIn delay-0_3">
        <div className="terminal-wrapper">
            ...
            <div id="terminal-wrapper" className="terminal-bot">
                ...
                <span className="">(Question {questionCounter}</span>): <span style={{fontSize: "45px"}}>{inputOne} + {inputTwo} &times; {inputThree}</span>
                </p>
                <p className="terminal-prompt mt-25 last-login">
                <span className="terminal-green">{d.getHours()}:{minutes}</span> &nbsp;
                <input
